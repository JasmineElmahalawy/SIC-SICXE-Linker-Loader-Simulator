...
class SicXE:
    def __init__(self) -> None:
        ...
        print("hello")
        self.labelframe = LabelFrame(menu, text="GENERATE LINKING LOADER FROM SIC-XE PROGRAM", font=("Arial", 16, "bold"),
                                     padx=50, pady=50)
        self.labelframe.pack()
        self.labelframe.config(bg="YELLOW", fg="GREEN")
        lb = Label(self.labelframe, text="Enter starting address")
        lb.pack()
        lb.config(bg="YELLOW", fg="BLACK")
        self.starting_address_entry = Entry(self.labelframe)
        self.starting_address_entry.pack()
        btn = Button(self.labelframe, text="Confirm address", command=self.setEntry)
        btn.pack()
        btn.config(bg="YELLOW", fg="BLACK")
        print("star:", self.starting_address)
        lb1 = Label(self.labelframe, text="Import HDRTME file .txt")
        lb1.pack()
        lb1.config(bg="YELLOW", fg="BLACK")
